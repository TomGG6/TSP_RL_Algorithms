...
# from utils.reversion_operator import adaptive_large_neighborhood_search
...
def run_vsr_lkh_algorithm(graph, algorithm, epsilon=0.4, beta=0.99, alpha=0.1, gamma=0.9, M=1, num_M=3, is_vsr=True, optimum=None):
    ...
    for trial in range(max_trials):
        ...
        better_tour = lkh_optimizer(better_tour, graph, algorithm)
...
def run_vsr_expected_lkh_algorithm(graph, algorithm, epsilon=0.4, beta=0.99, alpha=0.1, gamma=0.9, M=1, num_M=3, is_vsr=True, optimum=None):
    graph = convert_to_list(graph)
    start_time = time.time()
    max_trials = len(graph)
    max_num = int(max_trials / 20)
